...
public class LargePhotoWidgetProvider extends AppWidgetProvider {
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] iArr) {
        ...
        AppWidgetData appWidgetData = new AppWidgetData(2, MyAppConstants.Widget_Type_Id, -1,"", MyAppConstants.Temp_Id, MyAppConstants.IS_SIM_CARD);
    ...
    public static void updateWidgetView(int p, Context context, Intent intent) {
        AppWidgetManager appWidgetManager;
        int Pos = p;
        Context con = context;
        Class<LargePhotoWidgetProvider> aClass = LargePhotoWidgetProvider.class;
        AppDatabaseHelper helper = new AppDatabaseHelper(context);
        boolean booleanExtra = intent.getBooleanExtra(FlipWidget, false);
        AppWidgetMaster mainWidget = helper.getWidgetMaster(Pos);
        if (Pos != -1) {
            ...
            if (LeftClick.equals(intent.getAction()) || RightClick.equals(intent.getAction())) {
                ...
                if (mainWidget == null || !mainWidget.isFlipControl()) {
                    appWidgetManager = instance2;
                    Intent intent2 = new Intent(con, PhotoWidgetActivity.class);
                    intent2.putExtra(MyAppConstants.WidgetId, Pos);
                    ...
                    PendingIntent activity = PendingIntent.getActivity(con, 0, intent2, PendingIntent.FLAG_MUTABLE | PendingIntent.FLAG_UPDATE_CURRENT);
                    remoteViews.setViewVisibility(R.id.LlFlipperPaging, View.GONE);
                    remoteViews.setOnClickPendingIntent(R.id.RlPhoto1, activity);
                } else {
                    Intent intent3 = new Intent(con, PhotoWidgetActivity.class);
                    intent3.putExtra(MyAppConstants.WidgetId, Pos);
                    ...
                    PendingIntent activity2 = PendingIntent.getActivity(con, 0, intent3, PendingIntent.FLAG_MUTABLE | PendingIntent.FLAG_UPDATE_CURRENT);
                    Intent intent4 = new Intent(con, aClass);
                    intent4.setAction(LeftClick);
                    intent4.putExtra(FlipWidget, true);
                    intent4.putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, Pos);
                    appWidgetManager = instance2;
                    intent4.setData(Uri.parse(MyAppConstants.getUniqueId()));
                    PendingIntent broadcast = PendingIntent.getBroadcast(con, 0, intent4, PendingIntent.FLAG_MUTABLE | PendingIntent.FLAG_UPDATE_CURRENT);
                    Intent intent5 = new Intent(con, aClass);
                    intent5.setAction(RightClick);
                    intent5.putExtra(FlipWidget, true);
                    intent5.putExtra("appWidgetId", Pos);
                    intent5.setData(Uri.parse(MyAppConstants.getUniqueId()));
                    PendingIntent broadcast2 = PendingIntent.getBroadcast(con, 0, intent5, PendingIntent.FLAG_MUTABLE | PendingIntent.FLAG_UPDATE_CURRENT);
            ...
            Intent intent8 = new Intent(con, LargePhotoWidgetService.class);
            intent8.putExtra("WidgetID", Pos);
            ...
            appWidgetManager.notifyAppWidgetViewDataChanged(Pos, R.id.FlipView);

            appWidgetManager.updateAppWidget(Pos, remoteViews);
        }
    }
